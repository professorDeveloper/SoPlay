...
import com.azamovme.soplay.repository.TvRepository
import com.azamovme.soplay.utils.Color
import com.azamovme.soplay.utils.Utils.getJsoup
import com.azamovme.soplay.utils.printlnColored
...
class TvRepositoryImpl : TvRepository {
    ...
    override fun getTv() = flow<Result<ArrayList<Movie>>> {
        ...
        if (movieElements != null) {
            ...
            printlnColored("Uzbek channels not found.", com.azamovme.soplay.utils.Color.YELLOW)
        }
        movieElements.map {
            val href = it.select("a[href]").attr("href")
            val title = it.select("a[href]").text()
